...
Write a query to print the contest_id, hacker_id, name, and sums of total_submissions, 
total_accepted_submissions, total_views, and the total_unique_views for each contest sorted by contest_id. 
Exclude the contest from result if all four sums are 0. */
...
SUM(ss.total_accepted_submissions) AS total_accepted_submissions FROM Submission_Stats AS ss GROUP BY ss.challenge_id) AS sg
